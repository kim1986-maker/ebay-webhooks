...
const TOKEN = (process.env.EBAY_VERIFICATION_TOKEN_KSMART || "").trim();
...
export default async function handler(req, res) {
  ...
  if (incoming && TOKEN && incoming !== TOKEN) {
    console.log("[KSmart] Token mismatch", { incomingToken: incoming });
    return res.status(401).json({ ok: false });
  }

  console.log("[KSmart] MAD webhook payload:", body);
